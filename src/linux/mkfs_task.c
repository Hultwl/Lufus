...
int lufus_format(const char *dst, const LufusMkfsOpts *o,
  ...
  if (!strcmp(o->fs, "vfat") || !strcmp(o->fs, "fat32")) {
    if (!lufus_have("/usr/bin/mkfs.vfat")) { snprintf(err, cap, "mkfs.vfat missing"); return -1; }
    int i = 0;
    ...
    if (!lufus_have("/usr/bin/mkfs.ntfs")) { snprintf(err, cap, "mkfs.ntfs missing (ntfsprogs)"); return -1; }
    int i = 0;
    ...
    if (!lufus_have("/usr/bin/mkfs.exfat")) { snprintf(err, cap, "mkfs.exfat missing"); return -1; }
    int i = 0;
    ...
    if (!lufus_have("/usr/bin/mkfs.udf")) { snprintf(err, cap, "mkfs.udf missing"); return -1; }
